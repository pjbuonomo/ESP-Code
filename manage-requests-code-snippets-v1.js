function retrieveGroupMembers(initialValue) {
    ...
    fetch(endpointUrl, {
      ...
      .then((data) => {
        ...
        // Add "Unassigned" option if initialValue is undefined or null
        if (initialValue === undefined || initialValue === null) {
          members.unshift({ name: "Unassigned", userId: null });
        }
        ...
        const initialOption = Array.from(assignedToDropdown.options).find(
          (option) => option.value === String(initialValue)
        );
        if (initialOption) {
        ...
        assignedToDropdown.addEventListener("change", () => {
          const selectedOption =
            assignedToDropdown.options[assignedToDropdown.selectedIndex];
          const inputElement = document.querySelector(
            "span.dataQuickLocate[data-internalName='AssignedToId'] input"
          );
